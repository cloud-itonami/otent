#!/usr/bin/env nbb
;; Derived spatial-density (per-cell grid) table over Panoramax open
;; street imagery metadata — one source, one area, one derived task
;; per run.
;;
;;   nbb --classpath src bin/panorama_density.cljs --bbox 139.765 35.678 139.77 35.682
;;   nbb --classpath src bin/panorama_density.cljs --fixture payload.json --bbox W S E N
;;
...
;; unchanged. This script derives the density table and stores BOTH
    ...
    (if-not (:ok? result)
      ...
      (do (println (str "counts: fetched=" (:fetched (:counts result))
          ...
          (let [prov (px/provenance {:area-id (:area-id (px/check-bbox bbox))
                ...
                table (pxd/density-table {:observations (:observations result)
                                          :counts (:counts result)
                                          :provenance prov})
                derived-prov (pxd/provenance prov {:run-at retrieved-at})]
...
(defn- object-key [area-id]
  (str "otent/street-px-density/" area-id "/density-"
...
(defn- script-args []
  (let [argv (js->clj js/process.argv)
        i (first (keep-indexed (fn [idx a]
                                 (when (str/ends-with? a "panorama_density.cljs") idx))
                               argv))]
