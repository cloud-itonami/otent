(ns analyze
  "One bounded analysis pass over the already-licensed Otent imagery asset.
    ...
    nbb --classpath src bin/analyze.cljs --source modis-terra-truecolor \\
        --date 2026-08-29 --max-zoom 4 --out ledger/analysis

  The pure half lives in `otent.analysis` (and `otent.basemap` for the
  plan); this file is only I/O: fetch the tiles the ingest bound allows,
  decode them, classify, and write the derived table. Failures become
  :failed rows -- they are never dropped, because a table that hides its
  holes is not a table, it is a claim."
  (:require ["fs" :as fs]
            ["path" :as path]
            ["crypto" :as crypto]
            ["jpeg-js" :as jpeg]
...
(defn- artifact-hash []
  ...
  (sha256 (fs/readFileSync (path/join "src" "otent" "analysis.cljs"))))

;; ------------------------------------------------------------------ I/O

(defn- fetch-bytes [url]
  (-> (js/fetch url #js {:headers #js {"user-agent" "otent-analysis/0.1 (cloud-itonami)"}})
      (.then (fn [r]
               (if-not (.-ok r)
...
(defn- classify-one! [{:keys [tile url key]} source date]
  (-> (fetch-bytes url)
      (.then (fn [r]
               (if-not (:ok? r)
                 (an/failed-observation tile key (:error r) (:detail r))
                 (try
                   (let [img (.decode jpeg (:buf r) #js {:useTArray true})
                         ...
                         prov (an/provenance-record {:source source :date date :tile tile
                                                     :sha256 (sha256 (:buf r)) :key key}
                                                    (artifact-hash) "node/nbb (ClojureScript)")]
                     (an/observation prov metrics))
                 (catch :default e
                   (an/failed-observation tile key :decode/jpeg (str (.-message e))))))))))
...
(defn- run! [{:keys [source date tiles] :as plan} out-dir max-z]
  (let [start (js/Date.now)
        ...
        total (count tiles)]
    (log "analysis:" (:id source) "capture date" date
      ...
      (-> (js/Promise.all (clj->js (repeatedly conc worker)))
          (.then (fn [_]
                   (let [rows* (sort-by (comp (juxt :z :x :y) :tile :provenance) @rows)
                         ...
                         summary (an/run-summary {:source source :date date :max-z max-z}
                     ...
                     (let [back (->> (str/split (str (fs/readFileSync (path/join out "observations.jsonl") "utf8")) "\n")
                                     (remove str/blank?)
...
(defn- usage []
  (log "nbb --classpath src bin/analyze.cljs --source modis-terra-truecolor --date YYYY-MM-DD [--max-zoom 4] --out ledger/analysis"))
...
(defn- main []
  (let [args (js->clj (.-argv js/process))
        ...
        date (getopt "--date" nil)
        max-z (js/parseInt (getopt "--max-zoom" "4") 10)
        out (getopt "--out" "ledger/analysis")
        source (bm/source-for source-id)
        refusal (an/analysis-refusal source max-z date)]
    (if refusal
