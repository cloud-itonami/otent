#!/usr/bin/env nbb
;; Ingest one bounded area of KartaView (OpenStreetCam) open street
;; imagery metadata into R2, with the checks otent.kartaview declares.
;;
;;   nbb --classpath src bin/kartaview.cljs --bbox 139.765 35.678 139.77 35.682
;;   nbb --classpath src bin/kartaview.cljs --fixture payload.json --bbox W S E N
;;
...
(defn- normalize!
  ...
  refusal; per-photo refusals are counted, never dropped."
                       ...
                       (do (println (str "input sha256=" sha))
                           (-> (normalize! payload (:bbox args) retrieved-at)
                               (.then (fn [{:keys [ok? code error detail result]}]
                                        (if-not ok?
                                          (js/Promise.resolve {:ok? false :code code :error error :detail detail})
                                          (store! (:area-id (kv/check-bbox (:bbox args)))
                                                  (:bbox args) retrieved-at sha result))))))))))))))

;; Under nbb, process.argv is [node, nbb, script, ...user-args] (though
;; `nbb -e` puts the script elsewhere) — find this script by name and
;; take what follows, like `bin/street.cljs` does.
(defn- script-args []
  (let [argv (js->clj js/process.argv)
        i (first (keep-indexed (fn [idx a]
                                 (when (str/ends-with? a "kartaview.cljs") idx))
                               argv))]
