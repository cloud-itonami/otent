#!/usr/bin/env nbb
;; Derived temporal-coverage (vintage) table over Panoramax open street
;; imagery metadata — one source, one area, one derived task per run.
;;
;;   nbb --classpath src bin/panoramax_coverage.cljs --bbox 139.765 35.678 139.77 35.682
;;   nbb --classpath src bin/panoramax_coverage.cljs --fixture payload.json --bbox W S E N
;;
...
;; unchanged. This script derives the coverage table and stores BOTH
...
(defn- object-key [area-id]
  (str "otent/panoramax-coverage/" area-id "/vintage-"
...
(defn- store!
  [{:keys [area-id bbox retrieved-at input-sha256 result table derived-prov]}]
...
(defn- script-args []
  (let [argv (js->clj js/process.argv)
        i (first (keep-indexed (fn [idx a]
                                 (when (str/ends-with? a "panoramax_coverage.cljs") idx))
                               argv))]
