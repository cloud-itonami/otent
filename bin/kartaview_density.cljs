...
;;
;; The upstream normalization (privacy gate: provider-BLURRED processed
;; imagery only, public + active; GeoJSON lon/lat; unknowns visible)
;; belongs to otent.kartaview and is reused unchanged. This script
;; derives the density table and stores BOTH the observations and the
      ...
      (do (println (str "counts: fetched=" (:fetched (:counts result))
          ...
          (let [prov (kv/provenance {:area-id (:area-id (kv/check-bbox bbox))
            ...
            (println (str "density: grid="
                          ...
                          " pictures-placed=" (:placed (:table/pictures table))
                          " unplaceable=" (:unplaceable (:table/pictures table))))
            (js/Promise.resolve
             {:ok? true
              :sha input-sha256
...
(defn- object-key [area-id]
  (str "otent/kartaview-density/" area-id "/density-"
