#!/usr/bin/env nbb
;; Derived spatial-density (per-cell grid) table over KartaView open
;; street imagery metadata — one source, one area, one derived task
;; per run.
;;
;;   nbb --classpath src bin/kartaview_density.cljs --bbox 139.765 35.678 139.77 35.682
;;   nbb --classpath src bin/kartaview_density.cljs --fixture payload.json --bbox W S E N
;;
;; The upstream normalization (privacy gate: provider-BLURRED imagery
;; only; public + active; lon/lat order; unknowns visible) belongs to
;; otent.kartaview and is reused unchanged. This script derives the
;; density table and stores BOTH the observations and the derived
;; table with full provenance. Without $CF_CATALOG_TOKEN nothing is
;; written (exit 2): a refusal to write is reported, never faked.
;;
;; Exit 0 ingested · 1 refused (bounds violated / upstream changed) ·
;; 2 could not act (no credential, network unreachable, write failed).

(ns bin.kartaview-density
...
(defn- load-fixture!
  ...
  (if (or (not fixture) (not bbox))
    (js/Promise.resolve
     {:ok? false :code 2 :error :kartaview/missing-args
          ...
          (let [txt (str (fs/readFileSync fixture "utf8"))]
            (js/Promise.resolve
             {:ok? true :buf (js/Buffer.from txt "utf8")
              :payload (js/JSON.parse txt)}))
          (catch :default e
            (js/Promise.resolve {:ok? false :code 2 :error :kartaview/fixture-unreadable
                                 :detail (str (.-message e))})))))))
...
(defn- normalize-and-derive!
  ...
  (let [result (kv/normalize-payload (js->clj payload) {:bbox bbox :retrieved-at retrieved-at})]
    (if-not (:ok? result)
      ...
      (do (println (str "counts: fetched=" (:fetched (:counts result))
                        ...
                        " has-more=" (:has-more-data (:counts result))))
          (doseq [r (:refusals result)]
            (println (str "  REFUSED " (name (:error r)) ": " (:detail r))))
          (let [prov (kv/provenance {:area-id (:area-id (kv/check-bbox bbox))
                                     :bbox bbox
                                     :retrieved-at retrieved-at
                                     :input-sha256 input-sha256})
                table (kvd/density-table {:observations (:observations result)
                                          :counts (:counts result)
                                          :provenance prov})
                derived-prov (kvd/provenance prov {:run-at retrieved-at})]
            (println (str "density: grid="
                          (:cells-x (:table/grid table)) "x" (:cells-y (:table/grid table))
                          " photos-placed=" (:placed (:table/photos table))
                          " unplaceable=" (:unplaceable (:table/photos table))))
            (js/Promise.resolve
             {:ok? true
              :area-id (:area-id (kv/check-bbox bbox))
              :bbox bbox
              :retrieved-at retrieved-at
              :result result
              :table table
...
(defn- object-key [area-id]
  (str "otent/street-kv-density/" area-id "/density-"
...
(defn- store!
  ...
  (let [prov (kv/provenance {:area-id area-id :bbox bbox
                             :retrieved-at retrieved-at
    ...
    (-> ((if (:fixture args) load-fixture! fetch-payload!) args)
        (.then (fn [{:keys [ok? code error detail buf payload]}]
                 (if-not ok?
                   ...
                   (let [sha (sha256 buf)
                         {:keys [ok? error detail]} (kv/check-bbox (:bbox args))]
                     (if-not ok?
                       (js/Promise.resolve {:ok? false :code 1 :error error :detail detail})
...
(defn- script-args []
  (let [argv (js->clj js/process.argv)
        i (first (keep-indexed (fn [idx a]
                                 (when (str/ends-with? a "kartaview_density.cljs") idx))
                               argv))]
