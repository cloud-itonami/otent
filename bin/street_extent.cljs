...
;;
;;   nbb --classpath src bin/street_extent.cljs --fixture payload.json --area-id <area>
;;   MAPILLARY_ACCESS_TOKEN=... nbb --classpath src bin/street_extent.cljs \
...
;; This script derives the per-kind spatial extent + refusal ledger and
  ...
  (let [analysis (street/analyze (js->clj payload)
    ...
    (doseq [row (:per-kind t)]
      (println (str "  " (:kind row) "\t" (:count row)
                    "\tlon [" (:min-lon row) ", " (:max-lon row) "]"
                    "\tlat [" (:min-lat row) ", " (:max-lat row) "]"
                    "\t(unknown lon=" (:lon-unknown row)
                    " lat=" (:lat-unknown row) ")")))
    ...
    (println (str "ledger-reconciled=" (get-in t [:counts :ledger-reconciled?])
                  " lon-unknown=" (get-in t [:unknown-counts :lon-unknown])
                  " lat-unknown=" (get-in t [:unknown-counts :lat-unknown])))
