#!/usr/bin/env nbb
;; Derived temporal-coverage (vintage) table over KartaView open street
;; imagery metadata — one source, one area, one derived task per run.
;;
;;   nbb --classpath src bin/street_coverage.cljs --bbox 139.765 35.678 139.77 35.682
;;   nbb --classpath src bin/street_coverage.cljs --fixture payload.json --bbox W S E N
;;
...
;; coverage table and stores BOTH the observations and the derived
    ...
    (if-not (:ok? result)
      ...
      (do (println (str "counts: fetched=" (:fetched (:counts result))
          ...
          (let [prov (kv/provenance {:area-id (:area-id (kv/check-bbox bbox))
                ...
                table (sc/vintage-table {:observations (:observations result)
                                         :counts (:counts result)
                                         :provenance prov})
                derived-prov (sc/provenance prov {:run-at retrieved-at})]
...
(defn- object-key [area-id]
  (str "otent/street-coverage/" area-id "/vintage-"
...
(defn- script-args []
  (let [argv (js->clj js/process.argv)
        i (first (keep-indexed (fn [idx a]
                                 (when (str/ends-with? a "street_coverage.cljs") idx))
                               argv))]
