#!/usr/bin/env nbb
;; Derived spatial-density table over Mapillary street imagery
;; METADATA — one source, one area, one derived task per run.
;;
;;   MAPILLARY_ACCESS_TOKEN=... nbb --classpath src bin/mapillary_density.cljs \
;;     --bbox W S E N
;;   nbb --classpath src bin/mapillary_density.cljs --fixture payload.json \
;;     --bbox W S E N
;;
...
;; script derives the density grid and stores BOTH the observations
    ...
    (if-not (:ok? result)
      ...
      (do (println (str "counts: fetched=" (get-in result [:counts :fetched])
          ...
          (let [checked (mimg/check-bbox bbox)
                ...
                table (md/density-table {:observations (:observations result)
                                         :counts (:counts result)
                                         :provenance prov})
                derived-prov (md/provenance prov {:run-at retrieved-at})]
            (println (str "density: grid=" (get-in table [:table/grid :cells-x])
                          "x" (get-in table [:table/grid :cells-y])
                          " placed=" (get-in table [:table/images :placed])
                          " unplaceable=" (get-in table [:table/images :unplaceable])))
            (doseq [c (:table/cells table)]
              (println (str "  cell " (pr-str (:cell-index c))
                            " images=" (:images c)
                            " pano=" (:panoramas c)
                            " compass-unknown=" (:compass-unknown c))))
            (js/Promise.resolve
             {:ok? true
...
(defn- object-key [area-id]
  (str "otent/mapillary-density/" area-id "/density-"
...
(defn- script-args []
  (let [argv (js->clj js/process.argv)
        i (first (keep-indexed (fn [idx a]
                                 (when (str/ends-with? a "mapillary_density.cljs") idx))
                               argv))]
