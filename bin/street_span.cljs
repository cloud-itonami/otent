#!/usr/bin/env nbb
;; Derived per-kind temporal span over provider-published street
;; detections (Mapillary `map_features`) — one source, one area, one
;; derived task per run.
;;
;;   nbb --classpath src bin/street_span.cljs --fixture payload.json --area-id <area>
;;   MAPILLARY_ACCESS_TOKEN=... nbb --classpath src bin/street_span.cljs \
...
;; This script derives the per-kind temporal span + refusal ledger and
;; prints the report. No writes without an explicit store step elsewhere;
  ...
  (let [analysis (street/analyze (js->clj payload)
    ...
    (doseq [row (:per-kind t)]
      (println (str "  " (:kind row) "\t" (:count row)
                    "\tfirst-seen>=" (:earliest-first-seen-at row)
                    "\tlast-seen<=" (:latest-last-seen-at row)
                    "\t(unknown first=" (:first-seen-unknown row)
                    " last=" (:last-seen-unknown row) ")")))
    ...
    (println (str "ledger-reconciled=" (get-in t [:counts :ledger-reconciled?])
                  " first-seen-unknown=" (get-in t [:unknown-counts :first-seen-unknown])
                  " last-seen-unknown=" (get-in t [:unknown-counts :last-seen-unknown])))
