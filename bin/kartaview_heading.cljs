#!/usr/bin/env nbb
;; Derived heading/panorama coverage over KartaView open street imagery
;; metadata — one source, one area, one derived task per run.
;;
;;   nbb --classpath src bin/kartaview_heading.cljs --bbox 139.765 35.678 139.77 35.682
;;   nbb --classpath src bin/kartaview_heading.cljs --fixture payload.json --bbox W S E N
;;
...
;; heading table and stores BOTH the observations and the derived
  ...
  (let [result (kv/normalize-payload (js->clj payload) {:bbox bbox :retrieved-at retrieved-at})]
    (if-not (:ok? result)
      ...
      (do (println (str "counts: fetched=" (:fetched (:counts result))
          ...
          (let [bbox-check (kv/check-bbox bbox)
                prov (kv/provenance {:area-id (:area-id bbox-check)
                                     :bbox bbox
                                     :retrieved-at retrieved-at
                                     :input-sha256 input-sha256})
                table (kh/heading-table {:observations (:observations result)
                                         :counts (:counts result)
                                         :provenance prov})
                derived-prov (kh/provenance prov {:run-at retrieved-at})
                checks (kh/provenance-checks table)]
            (println (str "heading: " (pr-str (:table/heading-histogram table))
                          " heading-unknown=" (:heading-unknown (:table/photos table))))
            (if-not (:ok? checks)
              (js/Promise.resolve {:ok? false :code 2 :error (:error checks)
...
(defn- object-key [area-id]
  (str "otent/kartaview-heading/" area-id "/heading-"
...
(defn- script-args []
  (let [argv (js->clj js/process.argv)
        i (first (keep-indexed (fn [idx a]
                                 (when (str/ends-with? a "kartaview_heading.cljs") idx))
                               argv))]
