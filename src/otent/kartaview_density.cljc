(ns otent.kartaview-density
  "Derived spatial-density analysis over KartaView open street imagery
  METADATA, one derived task per run per the vision scope
  (:derived-allow :kartaview-open-data; one source, one area, one PR)."
  ;; What this task is: from imagery-asset observations already
  ;; normalized by otent.kartaview (provider-BLURRED, public, active
  ;; photos only), bin the one <=0.01 deg area the run fetched into a
  ;; fixed deterministic grid and count admissible photos per cell.
  ;;
  ...
  ;; - the grid is a LOWER BOUND: the provider pages results
  ;;   (hasMoreData, counted not followed); an empty cell proves
  ...
  ;; - every unknown stays visible: a photo whose published heading is
  ;;   not a number is still counted in its cell, as heading-unknown;
  ;;   a cell with zero admissible photos is kept as an explicit zero,
...
(defn density-table
  ...
  gated upstream by otent.kartaview: provider-BLURRED, public, active,
  inside the declared bbox). `counts` is the run's refusal/visibility
  ...
  (let [bbox (get-in provenance [:provenance/parameters :bbox])
        ...
        binned (reduce (fn [acc o]
                         (let [c (cell-of o shape)]
                           (if (nil? c)
                             ...
                             (let [k (get acc c)]
                               (assoc acc c
                                      (cond-> (assoc k :photos (inc (:photos k)))
                                        (heading-known? o)
    ...
    {:table/task-id task-id
     ...
     "photo positions are as published by the provider (lon/lat, with gpsAccuracy as the per-photo spatial-uncertainty figure, which the provider does not guarantee as an error bound); a count in one cell is not density of anything but admissible metadata in one fetched page"
...
(defn provenance
  ...
  (assoc upstream
         ...
         "input observations were gated upstream (only provider-BLURRED, public, active photos admitted); no pixel was fetched or stored; no face, plate, person or vehicle entity exists in this task"))
...
(defn provenance-checks
  ...
  the document carries, and the per-cell photo counts must sum to the
  placed count."
  [doc]
  (let [obs (get doc "observations")
        t (get doc "derived-table")
        n (count obs)
        placed (get-in t ["photos" "placed"])
        unplaceable (get-in t ["photos" "unplaceable"])
        cell-sum (reduce + 0 (map #(get % "photos") (get t "cells" [])))]
