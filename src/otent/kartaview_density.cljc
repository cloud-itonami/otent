(ns otent.kartaview-density
  "Derived spatial-density analysis over KartaView (OpenStreetCam) open
  street-imagery METADATA, one derived task per run per the vision
  scope (:derived-allow :kartaview-open-data; one source, one area, one
  PR). This is the KartaView counterpart of the Panoramax density task
  (`panorama_density.cljc`, `panoramax-street-density-v1`): the one
  street source that carried metadata, vintage, heading and a pixel
  sample but no per-cell density grid.

  What this task is: from imagery-asset observations already
  normalized by otent.kartaview (provider-BLURRED processed imagery
  only, public + active, uploader EXIF redacted, inside the declared
  bbox), bin the one <=0.01 deg area the run fetched into a fixed
  deterministic grid and count admissible pictures per cell.

  What this task is NOT:
  - a per-cell count is an observation about published metadata in one
    fetched page, never about street conditions, road quality,
    accessibility, ownership, or current existence
  - the grid is a LOWER BOUND: the provider pages results (a
    `hasMoreData` flag, counted not followed); an empty cell proves
  ...
  - every unknown stays visible: a picture whose published heading is
    not a number is still counted in its cell, as heading-unknown; a
    picture with no sequence id counts sequence-unknown; a cell with
    zero admissible pictures is kept as an explicit zero, never omitted

  The observation schema comes from otent.kartaview: `footprint` is the
  GeoJSON Point (lon/lat ordered), `orientation` is `{:heading-deg ...}`
  or :unknown, `sequence-id` is the provider's sequence string."
  (:require [otent.kartaview :as kv]))

;; ── task identity ────────────────────────────────────────────────────

(def task-id "kartaview-street-density-v1")
...
;; grid, whatever the observations are. Same target as the Panoramax
;; density task, so the two street-density tables are directly
;; comparable.
...
(defn- heading-known? [o]
  (let [orient (:observation/orientation o)]
    (boolean (and (map? orient) (number? (get orient :heading-deg))))))
...
(defn density-table
  "One normalized KartaView run -> one spatial-density table.

  `observations` are accepted imagery-asset observations (already
  gated upstream by otent.kartaview: provider-BLURRED processed imagery
  only, public + active, inside the declared bbox). `counts` is the
  run's refusal/visibility accounting, carried through unchanged so
  ...
  (let [bbox (get-in provenance [:provenance/parameters :bbox])
    ...
    {:table/task-id task-id
     ...
     "picture positions are as published by the provider (lon/lat, with gpsAccuracy in metres as the per-item spatial-uncertainty figure the provider states, not a guaranteed error bound); a count in one cell is not density of anything but admissible metadata in one fetched page"
...
(defn provenance
  ...
  (assoc upstream
         ...
         "input observations were gated upstream (provider-BLURRED processed imagery only, public + active, uploader EXIF redacted); no pixel was fetched or stored; no face, plate, person or vehicle entity exists in this task"))
