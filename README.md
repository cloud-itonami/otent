...
**天球 — the celestial sphere.** This repository is the *ingest actor* that
puts real public signals into the lake: it polls open feeds for satellites,
...
Nothing renders here. `cloud-itonami/app-tenkyu` draws the globe, and it
reads **only** what this actor has committed.

```bash
nbb --classpath src:../../kotoba-lang/sgp4/src bin/tenkyu.cljs feeds
nbb --classpath src:../../kotoba-lang/sgp4/src bin/tenkyu.cljs tick --dry-run
CF_CATALOG_TOKEN=... nbb --classpath src:../../kotoba-lang/sgp4/src bin/tenkyu.cljs tick
```
...
| table | rows | source |
|---|---|---|
| `cloud_itonami.tenkyu_quake` | 66 | USGS 2.5+ past day |
| `cloud_itonami.tenkyu_satellite` | 21 | CelesTrak GP (stations) |
| `cloud_itonami.tenkyu_aircraft` | 7,214 | OpenSky anonymous state vectors |
| `cloud_itonami.tenkyu_vessel` | — | **UNMEASURED**, see below |
| `cloud_itonami.tenkyu_fire` | — | **UNMEASURED**, see below |
...
```
tenkyu tick 2026-08-26T02:09:05.038Z
  committed 0  dry-run 3  nothing-new 0  refused 0  UNMEASURED 2  rows 0
  DRY-RUN celestrak -> tenkyu_satellite  would append 21 rows
  DRY-RUN usgs      -> tenkyu_quake      would append 33 rows
  DRY-RUN opensky   -> tenkyu_aircraft   would append 7233 rows
...
`tenkyu.governor`, which is pure — no network, no clock, no file handle,
...
**The 21 duplicate rows were not left in place.** `tenkyu_satellite` was
...
```
src/tenkyu/observation.cljc   the one row shape every feed lands in
src/tenkyu/governor.cljc      pure admit/hold
src/tenkyu/receipt.cljc       pure: run report and exit code
src/tenkyu/feeds/core.cljc    the registry, including what cannot be read
src/tenkyu/feeds/parse.cljc   payload -> observations, per feed. Pure.
bin/tenkyu.cljs               the only namespace that touches the network
