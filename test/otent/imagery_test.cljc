...
;; ---- the seventeenth bounded sample: MODIS Terra land surface
;; ---- temperature (day), one declared capture date, level 0, 1km matrix

(def lst-day-fixture-name "modis-terra-lst-day-20260903-z0.png")

(defn lst-day-fixture-path []
  (path/join (js/process.cwd) "test" "otent" "fixtures"
             lst-day-fixture-name))

(t/deftest lst-day-provenance-complete-test
  (t/is (true? (imagery/provenance-complete?
                imagery/modis-terra-lst-day-sample)))
  (t/testing "a dated record still carries every provenance key"
    (t/is (false? (imagery/provenance-complete?
                   (dissoc imagery/modis-terra-lst-day-sample
                           :capture-time))))))

(t/deftest lst-day-manifest-states-exactly-what-exists-test
  (let [m (imagery/manifest imagery/modis-terra-lst-day-sample)]
    (t/is (= (:asset-id imagery/modis-terra-lst-day-sample)
             (:asset-id m)))
    (t/is (re-find #"MODIS_Terra_Land_Surface_Temp_Day"
                   (:what-exists m)))
    (t/is (= "2026-09-03" (:capture-time m))
          "the declared capture date is carried verbatim")
    (t/is (true? (:level-0-only m))
...
(t/deftest lst-day-object-readback-test
  (t/testing "the lst-day fixture bytes on disk hash to what the record claims"
    (let [bytes (fs/readFileSync (lst-day-fixture-path))
          sha256 (-> (crypto/createHash "sha256")
                     (.update bytes)
                     (.digest "hex"))]
      (t/is (= (:payload-sha256
                imagery/modis-terra-lst-day-sample) sha256)))))

(t/deftest lst-day-verify-sample-test
  (let [bytes (fs/readFileSync (lst-day-fixture-path))
        sha256 (-> (crypto/createHash "sha256")
                   (.update bytes)
                   (.digest "hex"))
...
(t/deftest lst-day-licence-allowed-test
  (t/testing "the same allowlist gate applies to the lst-day source"
    (t/is (true? (imagery/licence-allowed?
...
;; ---- the eighteenth bounded sample: MODIS Terra land surface
;; ---- temperature (night), one declared capture date, level 0, 1km matrix

(def lst-night-fixture-name "modis-terra-lst-night-20260903-z0.png")

(defn lst-night-fixture-path []
  (path/join (js/process.cwd) "test" "otent" "fixtures"
             lst-night-fixture-name))

(t/deftest lst-night-provenance-complete-test
  (t/is (true? (imagery/provenance-complete?
                imagery/modis-terra-lst-night-sample)))
  (t/testing "a dated record still carries every provenance key"
    (t/is (false? (imagery/provenance-complete?
                   (dissoc imagery/modis-terra-lst-night-sample
                           :capture-time))))))

(t/deftest lst-night-manifest-states-exactly-what-exists-test
  (let [m (imagery/manifest imagery/modis-terra-lst-night-sample)]
    (t/is (= (:asset-id imagery/modis-terra-lst-night-sample)
             (:asset-id m)))
    (t/is (re-find #"MODIS_Terra_Land_Surface_Temp_Night"
                   (:what-exists m)))
    (t/is (= "2026-09-03" (:capture-time m))
          "the declared capture date is carried verbatim")
    (t/is (true? (:level-0-only m))
...
(t/deftest lst-night-object-readback-test
  (t/testing "the lst-night fixture bytes on disk hash to what the record claims"
    (let [bytes (fs/readFileSync (lst-night-fixture-path))
          sha256 (-> (crypto/createHash "sha256")
                     (.update bytes)
                     (.digest "hex"))]
      (t/is (= (:payload-sha256
                imagery/modis-terra-lst-night-sample) sha256)))))

(t/deftest lst-night-verify-sample-test
  (let [bytes (fs/readFileSync (lst-night-fixture-path))
        sha256 (-> (crypto/createHash "sha256")
                   (.update bytes)
                   (.digest "hex"))
...
(t/deftest lst-night-licence-allowed-test
  (t/testing "the same allowlist gate applies to the lst-night source"
    (t/is (true? (imagery/licence-allowed?
                  (:licence imagery/modis-terra-lst-night-sample))))
    (t/is (nil? (imagery/refusal
                 (:licence imagery/modis-terra-lst-night-sample))))))
...
;; ---- the twentieth bounded sample: MODIS Aqua land surface
;; ---- temperature (night), one declared capture date, level 0, 1km matrix

(def lst-night-aqua-fixture-name "modis-aqua-lst-night-20260905-z0.png")

(defn lst-night-aqua-fixture-path []
  (path/join (js/process.cwd) "test" "otent" "fixtures"
             lst-night-aqua-fixture-name))

(t/deftest lst-night-aqua-provenance-complete-test
  (t/is (true? (imagery/provenance-complete?
                imagery/modis-aqua-lst-night-sample)))
  (t/testing "a dated record still carries every provenance key"
    (t/is (false? (imagery/provenance-complete?
                   (dissoc imagery/modis-aqua-lst-night-sample
                           :capture-time))))))

(t/deftest lst-night-aqua-manifest-states-exactly-what-exists-test
  (let [m (imagery/manifest imagery/modis-aqua-lst-night-sample)]
    (t/is (= (:asset-id imagery/modis-aqua-lst-night-sample)
             (:asset-id m)))
    (t/is (re-find #"MODIS_Aqua_Land_Surface_Temp_Night"
                   (:what-exists m)))
    (t/is (= "2026-09-04" (:capture-time m))
          "the declared capture date is carried verbatim")
    (t/is (true? (:level-0-only m))
...
(t/deftest lst-night-aqua-object-readback-test
  (t/testing "the aqua lst-night fixture bytes hash to what the record claims"
    (let [bytes (fs/readFileSync (lst-night-aqua-fixture-path))
          sha256 (-> (crypto/createHash "sha256")
                     (.update bytes)
                     (.digest "hex"))]
      (t/is (= (:payload-sha256
                imagery/modis-aqua-lst-night-sample) sha256)))))

(t/deftest lst-night-aqua-verify-sample-test
  (let [bytes (fs/readFileSync (lst-night-aqua-fixture-path))
        sha256 (-> (crypto/createHash "sha256")
                   (.update bytes)
                   (.digest "hex"))
...
(t/deftest lst-night-aqua-licence-allowed-test
  (t/testing "the same allowlist gate applies to the aqua lst-night source"
    (t/is (true? (imagery/licence-allowed?
