;; Live verification of the GHRSST L4 MUR Sea Surface Temperature plan:
;; the URLs the plan produces must be exactly what the service answers.
;; Bounded: z2 = 21 tiles, one declared capture date. NOT sparse -- the
;; layer answers 200 with a coloured PNG over land as well as sea -- so
;; every answer must be 200-with-PNG-magic on the right key shape. A 404
;; here means the declared date has no published data yet (the analysis
;; publishes with a one-day lag), which is an error, not a hole.
(require '[otent.basemap :as bm]
         '[clojure.string :as str])

(def date "2026-08-30")
(def plan (bm/ingest-plan "ghrsst-mur-sst" 2 date))
(assert (:ok? plan))
                ...
                (fn [ab]
                  (let [b (js/Buffer.from ab)
                        ok? (and (= 200 status)
                                 (= 0x89 (aget b 0))
                                 (= 0x50 (aget b 1))
                                 (str/starts-with? key (str "otent/basemap/ghrsst-mur-sst/" date "/"))
