...
`bin/otent.cljk` (nbb) owns the orchestration; this owns the Iceberg
...
only for the part nbb cannot do, following the same split that
`com-junkawasaki/org-gleif-projections` already uses in this workspace.

## Append, not create-only

The GLEIF sibling refuses to write to an existing table, which is right for
a projection rebuilt whole. This is a time series: every poll appends to a
table that must already exist after the first one. So:

  --create   create the table if absent (first run for a kind)
...
it exactly. A feed that adds a column mid-stream stops the run; it does not
get null-filled into the existing shape, because that writes a column of
nulls that reads as "these rows had no value" rather than "this run could
not represent them".
...
def main() -> int:
            ...
            try:
                ...
                if not a.create:
                    print(
                        ...
                        "if this is the first run for this kind. Refusing to "
                        "create it implicitly: a mistyped --table would start a "
                        "second history that looks healthy and holds half the rows.",
                        file=sys.stderr,
                    )
                    return 1
    ...
    if table is None:
        # Zero rows creates a snapshot that says "a poll happened and found
        # nothing", which is indistinguishable from a poll that failed.
