;; Live verification of the MODIS Aqua plan: the URLs the plan
...
(def plan (bm/ingest-plan "modis-aqua-truecolor" 2 "2026-08-30"))
(assert (:ok? plan))
                     ...
                     (.then (fn [ab]
                              (let [b (js/Buffer.from ab)
                                    ok? (and (= 200 (.-status r))
                                             (= 0xFF (aget b 0))
                                             (= 0xD8 (aget b 1))
                                             (str/starts-with? key "otent/basemap/modis-aqua-truecolor/2026-08-30/")
