;; Live verification of the MODIS Terra Land Surface Temperature (Night)
;; plan: the URLs the plan produces must be exactly what the service
;; answers. Bounded: z2 = 21 tiles, one declared capture date. NOT
;; sparse -- the layer answers 200 with a coloured PNG even over ocean
;; -- so every answer must be 200-with-PNG-magic on the right key shape.
;; A 404 here means the declared date has no published data yet (the
...
(def date "2026-08-30")
(def plan (bm/ingest-plan "modis-terra-lst-night" 2 date))
(assert (:ok? plan))
                ...
                (fn [ab]
                  (let [b (js/Buffer.from ab)
                        ok? (and (= 200 status)
                                 (= 0x89 (aget b 0))
                                 (= 0x50 (aget b 1))
                                 (str/starts-with? key (str "otent/basemap/modis-terra-lst-night/" date "/"))
