;; Live verification of the MODIS Aqua Land Surface Temperature (Night)
...
(def date "2026-08-30")
(def plan (bm/ingest-plan "modis-aqua-lst-night" 2 date))
(assert (:ok? plan))
                ...
                (fn [ab]
                  (let [b (js/Buffer.from ab)
                        ok? (and (= 200 status)
                                 (= 0x89 (aget b 0))
                                 (= 0x50 (aget b 1))
                                 (str/starts-with? key (str "otent/basemap/modis-aqua-lst-night/" date "/"))
